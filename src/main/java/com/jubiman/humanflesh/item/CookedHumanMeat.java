...
public class CookedHumanMeat extends FoodConsumableItem {
	...
	public boolean consume(Level level, HungerMob hungerMob, InventoryItem item) {
		...
		if (consumed)
			if (hungerMob instanceof PlayerMob && ((PlayerMob) hungerMob).isServerClient())
				SanityPlayersHandler.getPlayer(((PlayerMob) hungerMob).getServerClient()).removeSanity(5);
		return consumed;
	}
	...
	public ListGameTooltips getTooltips(InventoryItem item, PlayerMob perspective, GameBlackboard blackboard) {
		...
		tooltips.add(GameColor.RED.getColorCode() + String.format(Localization.translate("buffmodifiers", "sanity"), -5));
		return tooltips;
	}
}
